...
启动桌面版后，点击顶部“导入 WeGame 天赋”，粘贴严格的公开分享链接并读取预览。应用前必须确认基础职业、升华、部分导入限制；覆盖非空 Build 和省略缺失武器节点也各自需要明确确认。取消、非法链接、网络错误或迟到响应都不会修改当前 Build。

当前经项目所有者实机验证的实验规则是 `set1` → 武器组 I、`set2` → 武器组 II。界面在替换前显示来源数、实际可应用数、省略节点 ID/来源/原因，以及按“通用 + max(武器组 I, 武器组 II)”计算的真实预算。当前公开样本为 I `16/17`、II `17/17`，节点 `52669` 因不在精简 Planner 目录而明确省略。这不是对所有未来 WeGame 数据的通用编号保证。

属性选择覆盖、珠宝内容、未解释字段和未解析证据不会激活或保存；装备、技能和 DPS 不在读取范围。已应用的受支持普通、升华、普通插槽和武器组天赋使用现有原生 Build 字段保存并可重新打开。
