...
当前唯一运行时默认文本源为 `python_agent/prompts.py`，版本 `chat-prompts-zh-v5`。
...
显式保存原子写入 v5，仅存储与当前默认不同的块，避免以后再把物化默认误认为自定义。
...
14 个工具描述总预算仍为 16000。空白原样保存，不自动补字、不截断输入。
用户自定义文本会随请求发送给所选模型，请勿填写秘密。编辑不授予新权限。

## 完整中文默认文本

下列 20 块按维护目录顺序完整列出。测试逐字校验文档与实际来源一致。
