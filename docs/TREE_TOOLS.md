...
Returns shortest allocatable path from the category's start set to the target. Path nodes list runs start→target. Each node: `{id, name, kind, alreadyAllocated: bool, isTarget: bool}`.

Result: `{path, pathLength, newNodesCount, newNodeIds, direction: "start_to_target", category, reachable: bool, snapshotId, warning?}`.

`startId` parameter returns `{reachable: false, unsupported: "startId"}` — custom-start path finding is not yet supported. Use the category's default start set.
...
| Bound | Value |
|--------|------|
| Max projected nodes | 2400 |
| Max projected edges | 6000 |
| Max IPC snapshot bytes | 2,000,000 |
| Max serialised snapshot bytes | 1,800,000 |
